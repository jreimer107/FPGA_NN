...
def decimalToBinary(num) : 
    ...
    if(len(binary_fract) <= fraction_width):
        binary_fract = binary_fract + (fraction_width-len(binary_fract)) * '0' 
...
if __name__ == "__main__" :
    ...
    hex_n = bin2hex(bin_n)
    print(n, bin_n, hex_n)
    for file in glob.glob('C:/Users/ECE_STUDENT/Downloads/output_camera.mat'):     
    #for file in glob.glob('C:/Users/ECE_STUDENT/Desktop/FPGA_NN/accelerator/camera_0_smallSTD.mat'):
        mat = scipy.io.loadmat(file)
        layer_weight = mat['bb'] # array
        ...
        for i in range(layer_weight.shape[0]):
            ...
            z = layer_weight[i].item()
            print(z)
            layer_weight[i] = Decimal(z).quantize(Decimal('1.000'), rounding=ROUND_UP)
            #layer1_weight[i][j] = float_bin(layer1_weight[i][j], 6)
            #print(layer_weight[i][j])
            bin_n = decimalToBinary(layer_weight[i])
            hex_n = bin2hex(bin_n)
            #binary_layer.append(bin_n)
            #print(binary_layer[i][j])
            #hex_n = bin2hex(bin_n)
            #hex_layer.append( bin2hex(bin_n))
            print(i, layer_weight[i], bin_n, hex_n, len(binary_layer))
            bin_to_write = ""
